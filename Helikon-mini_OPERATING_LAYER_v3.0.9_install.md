<!-- BEGIN HELIKON-MINI OPERATING LAYER INSTALLATION SCRIPT -->
...
## Purpose
- Extract one Helikon-mini Operating Layer memory at a time from the uploaded Operating Layer pack (SSOT).
...
- For the Operating Layer installer surface, treat the unified JSON install package as the installation SSOT.
  - Primary install artifact: `Helikon-mini_Install_Package_v3.0.9.json`
  - Human-readable projection/fallback: `Helikon-mini_OPERATING_LAYER_v3.0.9_install.md`
...
<!-- END HELIKON-MINI OPERATING LAYER INSTALLATION SCRIPT -->

---

# Helikon-mini Operating Layer v3.0.9 — install (Saved Memories pack)

This file preserves the JSON-first install surface, normalized installer command family, and `APPROVE` build token while advancing the synchronized package pointer to the v3.0.9 SETUP-to-INSTALL handoff hard-binding + synchronized-surface closure set and leaving payload text and runtime semantics unchanged.

**Memory content version:** v3.0.0.  
**Install surface version:** v3.0.9.

**Prerequisite:** Install the Helikon-mini System Layer first.

**IMPORTANT:** For `EXTRACT`, the payload is **everything between the `---` separators** for the target memory (including its `## n)` header).
